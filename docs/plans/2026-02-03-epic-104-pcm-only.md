...
**Goal:** HTTP API を `GET /healthz` + `POST /transcribe_pcm` のみに縮退し、外部デコーダ/外部メタ取得 依存と不要経路（CLI/話者分離/ファイル入力等）を完全に削除する。

**Architecture:** PCM(s16le, mono) のみを FastAPI で受け取り、`decode_pcm_s16le_bytes` → `transcribe_waveform(_chunked)` の最短経路へ収束させる。ファイル/ストリーム/話者分離/録音/サブプロセスの入口は削除し、依存関係とテストを PCM-only に最小化する。

**Tech Stack:** Python 3.13, FastAPI, mlx-whisper, numpy, unittest

---

### Task 1: ISSUE-1 API 縮退（`/transcribe` 削除）
...
git commit -m "🔧 /transcribe API を削除"
```

---

### Task 2: ISSUE-2 audio/inspection の削除
...
bash "$HOME/.codex/skills/epic-subissue-runner/scripts/run_issue_gates.sh"
git commit -m "🔧 audio inspection を削除"
```

---

### Task 3: ISSUE-3 外部デコーダ/外部メタ取得 依存の根絶（utils/pipeline/chunking/service ほか）
...
- Delete: `src/lib/diarize/*` (module 全体)

**Step 1: Write the failing test**
```python
# tests/test_外部デコーダ_strings.py
import subprocess
import unittest

class TestExternalDecoderStrings(unittest.TestCase):
    def test_外部デコーダ_grep_is_empty(self) -> None:
        result = subprocess.run(
            ["git", "grep", "-nE", r"\\b外部デコーダ\\b|\\b外部メタ取得\\b"],
            capture_output=True,
            text=True,
        )
...
**Step 2: Run test to verify it fails**
Run: `python -m unittest tests.test_外部デコーダ_strings -v`
...
# - remove _translate_decode_error 外部デコーダ branches
...
git add src/lib/audio/utils.py src/lib/asr/pipeline.py src/lib/asr/chunking.py src/lib/asr/service.py \
  src/lib/asr/main.py src/lib/asr/__init__.py tests/test_外部デコーダ_strings.py
rm -rf src/lib/asr/subproc.py src/lib/asr/streaming_input.py src/lib/asr/streaming_runtime.py src/lib/diarize
bash "$HOME/.codex/skills/epic-subissue-runner/scripts/run_issue_gates.sh"
git commit -m "🔧 外部デコーダ/外部メタ取得 依存を削除"
```

---

### Task 4: ISSUE-4 CLI/Makefile/Docs の PCM-only 化
...
```markdown
# AGENTS.md (該当セクションのみ更新)
- PCM-only 運用: HTTP は `POST /transcribe_pcm` のみ利用します。
- 開発: `uvicorn src.cmd.http:create_app --reload --port 8000 --factory`
...
bash "$HOME/.codex/skills/epic-subissue-runner/scripts/run_issue_gates.sh"
git commit -m "🔧 CLI/Makefile/Docs を PCM-only 化"
```

---

### Task 5: ISSUE-5 テスト刷新（/transcribe_pcm のみ担保）

**Files:**
- Create: `tests/test_http_transcribe_pcm.py`
- Delete/Trim: `tests/test_audio_*` (外部デコーダ/inspection 依存を削除)
...
bash "$HOME/.codex/skills/epic-subissue-runner/scripts/run_issue_gates.sh"
git commit -m "🔧 /transcribe_pcm テストを追加"
```

---

### Task 6: ISSUE-6 依存関係の最小化（pyproject/uv.lock）
...
Run:
- `git grep -nE '\\b外部デコーダ\\b|\\b外部メタ取得\\b'` (0件)
- `python -m unittest discover -s tests`
