...
help:
	...
	@echo "                               # ファイル入力で書き起こし"
	@echo "  make cli-stream [MODEL=...] [LANGUAGE=...] [TASK=...] [NAME=...]"
	@echo "                               # 標準入力経由でストリーム書き起こし (音声バイトをパイプで供給すること)"
	@echo "  make audio-streaming [DEVICE=idx] [SECONDS=] [MODEL=...] [LANGUAGE=...] [TASK=...] [NAME=...]"
...
audio-streaming:
	...
	if [ -n "$(TASK)" ]; then FLAGS="$$FLAGS --task $(TASK)"; fi; \
	if [ -n "$(NAME)" ]; then FLAGS="$$FLAGS --name $(NAME)"; fi; \
	echo "ffmpeg -f avfoundation -i $${DEVICE_VAL} -ac 1 -ar 16000 $$SECONDS_OPT -f wav - | $(CLI) stream $$FLAGS"; \
	ffmpeg -f avfoundation -i "$${DEVICE_VAL}" -ac 1 -ar 16000 $$SECONDS_OPT -f wav - \
		| $(CLI) stream $$FLAGS
